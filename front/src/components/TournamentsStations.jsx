...
function TOLoader() {
  ...
  const getCardSize = (totalCards) => {
    ...
    if (totalCards <= 16) return 6;
    return 12;
  };

  return (
    <Container sx={{ mt: 4, height: '100vh', display: 'flex', flexDirection: 'column' }}>
      {!submittedEventId ? (
              ...
              <>
                <Box sx={{ flexGrow: 1, overflow: 'auto' }}>
                  <Grid container spacing={2} sx={{ mt: 2 }}>
                    {tournamentData.event.sets.nodes
                      .slice((page - 1) * CARDS_PER_PAGE, page * CARDS_PER_PAGE)
                      .map(({ id, state, station, slots }) => (
                        <Grid item xs={getCardSize(tournamentData.event.sets.nodes.length)} key={id}>
                          <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', borderColor: state === 2 ? 'white' : state === 6 ? 'orange' : 'inherit', borderWidth: 2, borderStyle: 'solid' }}>
                            <CardContent sx={{ flexGrow: 1 }}>
                              ...
                              <Typography variant="body2" component="div" sx={{ color: state === 6 ? 'orange' : 'inherit' }}>
                ...
                <Box sx={{ mt: 2, mb: 2 }}>
                  <Pagination
                    count={Math.ceil(tournamentData.event.sets.nodes.length / CARDS_PER_PAGE)}
                    page={page}
